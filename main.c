...
void simpanDataTransaksi(int transaksi, int count, Transaksi items[],
                         float total, float diskon, float pajak,
                         float totalKeseluruhan, float pembayaran,
                         float kembalian) {
  ...
  if (file != NULL) {
    ...
    fprintf(file, "No Struk #%04d%02d%02d%d\n", localTime->tm_year + 1900,
            localTime->tm_mon + 1, localTime->tm_mday, transaksi);
    fprintf(file, "                     Sales Receipt\n\n");
    for (int i = 0; i < count; i++) {
      fprintf(file, "%s\n%dx \t\t@%.2f\t\t\t\tRp %.2f\n", items[i].namaBarang,
              items[i].jumlah, items[i].hargaBarang, items[i].hargaTotalBarang);
    }
    fprintf(file,
            "---------------------------------------------------------\n");
    fprintf(file,
            "Subtotal:Rp %.2f\nDiskon:Rp %.2f\nPajak:Rp %.2f\nTotal bayar:Rp "
            "%.2f\nPembayaran:Rp %.2f\nKembalian:Rp %.2f",
            total, diskon, pajak, totalKeseluruhan, pembayaran, kembalian);
    ...
    fprintf(file,
            "input kasir tidak dapat dikembalikan atau ditukar kembali\n");
    fprintf(file,
            "---------------------------------------------------------\n");
...
int main() {
  ...
  float total, hargaTotalBarang, pembayaran, pajak, totalKeseluruhan = 0.0,
                                                    diskon = 0.0;
  ...
  while (true) {
    ...
    do {
      ...
      if (result == -1) {
        ...
        fflush(stdin);
        printf("Barang: %s\nHarga:Rp %.2f\n", barang[result].namaBarang,
               barang[result].hargaBarang);
        printf("Jumlah: ");
        fscanf(stdin, "%d", &sum);
        fflush(stdin);
        while (sum <= 0) {
    ...
    simpanDataTransaksi(transaksi++, count, items, total, diskon, pajak,
                        totalKeseluruhan, pembayaran, kembalian);
  }
  return 0;
}